...
import pl.edu.agh.tai.persistence.PlaceDao;
...
public class PlaceController {
    ...
    public void createPlace(@RequestBody PlaceDto place) {
        placeDao.save(new PlaceEntity(place.getName(), place.getCity(), place.getStreet(), place.getBuildingNumber()));
    }

    @RequestMapping(path = "api/places", method = RequestMethod.GET)
    public List<Place> showPlaces() {
        Iterable<PlaceEntity> placeRepositoryAll = placeDao.findAll();
    ...
    public Place getPlace(@PathVariable(value = "id") long id) {
        ...
        PlaceEntity placeEntity = placeDao.findOne(id);
